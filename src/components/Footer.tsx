...
export async function Footer() {
  ...
  const boardTextureURLs = settings.data.footer_skateboards
    .map((item) => asImageSrc(item.skateboard, { h: 600 })) // provide the image URL
    .filter((url): url is string => Boolean(url));

  console.log("image passed: ", boardTextureURLs);
  console.log("image type: ", typeof boardTextureURLs);
